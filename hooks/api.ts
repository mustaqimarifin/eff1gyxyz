...
//export type AddCommentPayload = Database['public']['Tables']['comments']['Insert'];

export type CommentReactionMetadata =
  Database["public"]["Views"]["comment_reactions_metadata"]["Row"];
export type DisplayUser = Database["public"]["Views"]["display_users"]["Row"];
...
export const createApiClient = (supabase: SupabaseClient) => {
  const authorId = async () => (await supabase.auth.getUser()).data.user.id;

  const authuRRID = (user: Comment["user"]) => user.id;
  ...
  const removeCommentReaction = async ({
    ...
    const query = supabase
      .from("comment_reactions")
      .delete({})
      .match({ reaction_type, comment_id, user_id: authorId })
      .single();
